      ...
      <Card>
        <CardContent className="pt-6">
          <div className="space-y-4">
            {/* Search Bar */}
            <div>
              <label className="text-sm font-medium text-muted-foreground mb-2 block">Search by Student or User</label>
              <Input
                placeholder="Search..."
                value={search}
                    ...
                    <TableRow key={log.id}>
                      ...
                      <TableCell className="text-sm">
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">{log.subjectTitle}</div>
                        </div>
                      </TableCell>
                      <TableCell className="text-xs max-w-sm">
                        <div className="space-y-1">
                          {Object.entries(log.changes).map(([field, { old: oldVal, new: newVal }]) => (
                            <div key={field} className="font-mono bg-muted p-1 rounded text-[11px]">
                              <span className="font-semibold">{field}:</span>
                              {log.action === "UPDATE" ? (
                                <>
                                  {oldVal !== null && <span className="text-red-600"> {formatGradeValue(oldVal)}</span>}
                                  {oldVal === null && <span className="text-red-600"> N/A</span>}
                                  <span className="text-muted-foreground"> →</span>
                                  <span className="text-green-600"> {formatGradeValue(newVal)}</span>
                                </>
                              ) : (
                                <span className="text-green-600"> {formatGradeValue(newVal)}</span>
                              )}
